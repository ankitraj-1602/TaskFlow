...
class AttachmentService {
  async uploadAttachment(taskId, userId, file) {
    ...
    try {
      ...
      logger.warn('Failed to emit attachment:created', {
  ...
  async deleteAttachment(attachmentId, userId) {
    ...
    try {
      ...
      logger.warn('Failed to emit attachment:deleted', {
        attachmentId,
        error: socketError.message,
      });
    }
    ...
    logger.info('Attachment deleted', {
      ...
      deletedByManager: !isUploader && isManager,
    });
  ...
  deleteFileFromDisk(absolutePath) {
    try {
      ...
      // ⚠️ Non-fatal — DB delete is what matters, disk cleanup is best-effort
