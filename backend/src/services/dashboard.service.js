...
class DashboardService {
  async getWorkspaceStats(workspaceId, userId) {
    ...
    return cacheWrapper(cacheKey, 60, async () => {
      ...
      // ⚡ Log DB query time (only on cache miss) — useful for perf monitoring
  ...
  async getTaskTrends(workspaceId, userId, days = 30) {
    ...
    return cacheWrapper(cacheKey, 120, async () => {
      ...
      logger.debug('Dashboard trends computed', {
        workspaceId,
        userId,
        days,
        duration,
  ...
  async getTeamProductivity(workspaceId, userId) {
    ...
    return cacheWrapper(cacheKey, 60, async () => {
      ...
      logger.debug('Dashboard team productivity computed', {
        workspaceId,
        userId,
        duration,
  ...
  async getProjectProgress(workspaceId, userId) {
    ...
    return cacheWrapper(cacheKey, 60, async () => {
      ...
      logger.debug('Dashboard project progress computed', {
        workspaceId,
        userId,
        duration,
  ...
  async getOverdueBreakdown(workspaceId, userId) {
    ...
    return cacheWrapper(cacheKey, 60, async () => {
      ...
      logger.debug('Dashboard overdue breakdown computed', {
        workspaceId,
        userId,
        duration,
        overdueUserCount: rows.length,
      });
